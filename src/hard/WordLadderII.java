...
public class WordLadderII {
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
            ...
            for (int now : tmp) {
                for (int i = 0; i < len; i++) {
                    if (graph[now][i] == 1 && viewed[i] != 1){
                        li.add(i);
                        if (froms[i] == null) froms[i] = new HashSet<>();
                        froms[i].add(now);
                    }
                }
            }
        ...
        // 整理结果
        return getRes(froms, wordList, target, len-1).stream().distinct().collect(Collectors.toList());
    }
     ...
     * @return
     */
    private List<List<String>> getRes(Set<Integer>[] froms, List<String> wordList, int begin, int end){

...
public class WordLadderII {
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = i + 1; j < len; j++) {
                graph[i][j] = graph[j][i] = canTransfer(wordList.get(i), wordList.get(j));
            }
        }
        int target = wordList.indexOf(endWord);
        int[] viewed = new int[len];
        // 存放从哪可以到达这个点
        List<Integer>[] froms = new List[len];
        // 存放每一步可以到达的点
        ...
        // 循环探索每一步可以到达的点
        while (true){
            tmp = steps.get(steps.size()-1);
            if (viewed[target] == 1) break;
            List<Integer> li = new ArrayList<>();
            for (int now : tmp) {
                for (int i = 0; i < len; i++) {
                    if (graph[now][i] == 1 && viewed[i] != 1){
                        li.add(i);
                        if (froms[i] == null) froms[i] = new ArrayList<>();
                        froms[i].add(now);
                    }
                }
            }
            if (li.isEmpty()) return res;   // 空说明没有新增的点，之前判断过目标点尚未到达，因此图是非连通的，不可能到达
            for (int i : li) {
                viewed[i] = 1;
            }
            steps.add(li);
        }
        res = getRes(froms, wordList, target, len-1);
        System.out.println("sedfd");
        // 整理结果
    ...
    /**
     * 整理结果
     * @param froms 存放可以从哪些点移动到当前索引代表的点
     * @param wordList
     * @param begin 从哪个开始
     * @param end   到哪个结束
     * @return
     */
    private List<List<String>> getRes(List<Integer>[] froms, List<String> wordList, int begin, int end){
        List<List<String>> resTmp = new ArrayList<>();
        if (end == begin){
            List<String> one = new ArrayList<>();
            one.add(wordList.get(begin));
            resTmp.add(one);
            return resTmp;
        }
        for (int next : froms[begin]){
            for (List<String> l : getRes(froms, wordList, next, end)){
                l.add(wordList.get(begin));
                resTmp.add(l);
            }
        }
        return resTmp;
     ...
     * @param w1 单词1
     * @param w2 单词2
     * @return   1 可以转换 0 不能转换
     */
    private int canTransfer(String w1, String w2){
        int len = w1.length();
        int diff = 0;
        for (int i = 0; i < len; i++) {
